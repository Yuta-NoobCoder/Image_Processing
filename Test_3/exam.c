        ...
        for (int j = 0; j < width; j++) // Width
        {
            for (int k = 0; k < 3; k++) // CbCrY
            {
                ...
                if (y < height && x < width)
                {
                    ...
                    r0 = (1 - s) * imgin[k][fixed_y][fixed_x] + s * imgin[k][fixed_y][fixed_x + 1];
                    r1 = (1 - s) * imgin[k][fixed_y + 1][fixed_x] + s * imgin[k][fixed_y + 1][fixed_x + 1];

                    p = (1 - t) * r0 + t * r1;

                    imgout[k][i][j] = (unsigned char)(p + 0.5);
                }
                else
                {
                    // 範囲外なので黒で埋める
                    imgout[k][i][j] = k == 2 ? 0 : 128;
                }
            }
        }
    }
    //--------------------------右上, 領域A(左上128x128)の2倍拡大--------------------------//
    //拡大後の
    rate = 2.0;
    for (int i = 0; i < 256; i++) // Height
    {
        for (int j = 0; j < 256; j++) // Width
        {
            for (int k = 0; k < 3; k++) // CbCrY
            {
                ...
                if (y < 256 && x < 256)
                {
                    ...
                    imgout[k][i][j + 256] = (unsigned char)(p + 0.5);
                }
                else
                {
                    // 範囲外なので黒で埋める
                    imgout[k][i][j + 256] = k == 2 ? 0 : 128;
                }
            }
        }
    }
    //--------------------------左下, 領域B(中心の256x256)を移動--------------------------//
    for (int i = 128; i < 384; i++) // Height
    {
        for (int j = 128; j < 384; j++) // Width
        {
            for (int k = 0; k < 3; k++) //CbCrY
            {
                imgout[k][i + 128][j - 128] = imgin[k][i][j];
            }
        }
    }
    //--------------------------右下, 領域B(中心の256x256)を先鋭化--------------------------//
                ...
                for (int k = 0; k < 3; k++) //CbCrY
                {
                    if (k == 2) //Y信号のみ
                    {
                        ...
                        for (int l = i - 1; l <= i + 1; l++)
                        {
                            ...
                            {
                                sum += imgin[2][l][m] * factor[l - (i - 1)][m - (j - 1)];
                            }
                        }

                        //四捨五入して処理後の値とする
                        filtered_p = (int)(sum + 0.5);
                        //0から255以内に
                        if (filtered_p < 0)
                        {
                            imgout[2][i + 128][j + 128] = 0;
                        }
                        else if (filtered_p > 255)
                        {
                            imgout[2][i + 128][j + 128] = 255;
                        }
                        else
                        {

                            imgout[2][i + 128][j + 128] = filtered_p;
                        }
                        sum = 0;
                    }
                    else
                    {
                        imgout[k][i + 128][j + 128] = 128;
                    }
                }
            }
        }
