...
void processing(void)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        mean = ((double)levels[i] / max) * 100;
        printf("画素値%4d :%5d  -->%4d\n", i, levels[i], (int)(mean + 0.5));
    }
    ...
    {
        mean = ((double)levels[i] / max) * 100;
        levels[i] = (int)(mean + 0.5);
    }
    //塗りつぶし
    for (int i = height - 1; i >= height - 100; i--)
    {
    ...
    {
        for (int j = height - 1; j > height - levels[i] - 1; j--)
        {
            imgout[0][j][i] = 0xff; //cr
            imgout[1][j][i] = 0x80; //cb
            imgout[2][j][i] = 0x00; //y
        }
    }
}
