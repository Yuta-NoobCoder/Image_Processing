...
void processing(void)
{
    int sum = 0, level;
    double per_level;
    int threshold[16]; //各レベルの閾値

    //2, 4, 8, 16の場合のみ実行
    do
    {
        printf("出力画像の量子化レベルを入力してください(2, 4, 8, 16) : ");
        scanf("%d", &level);
    } while (level != 2 && level != 4 && level != 8 && level != 16);
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        for (int j = 0; j < width; j++)
        {
            for (int k = 0; k < 3; k++) //CbCrY
            {
                if (k == 2) //Y信号のみ
                {
                    if (imgin[k][i][j] <= threshold[0])
                    {
                        imgout[k][i][j] = threshold[0] / 2;
                    }
                    else
                    {
                        for (int l = level - 2; l >= 0; l--) //各レベルの閾値と比較
                        {
                            if (imgin[k][i][j] >= threshold[l])
                            {
                                ...
                                {
                                    imgout[k][i][j] = (threshold[l] + threshold[l + 1]) / 2;
                                }
                                break;
                            }
                        }
