...
void processing(void)
{
    ...
    for (int i = height - 1; i >= 0; i-=2) //Height
    {
        for (int j = 0; j < width; j+=2) //Width
        {
            for (int k = 0; k < 3; k++) //CbCrY
            {
                ...
                if(k == 2) 
                {
                    average = ( imgin[k][i][j] + imgin[k][i - 1][j] + imgin[k][i][j + 1] + imgin[k][i - 1][j + 1] ) / 4;
                    //四画素を平均値に置き換え
                    imgout[k][i][j] = imgout[k][i - 1][j] = imgout[k][i][j + 1] = imgout[k][i - 1][j + 1] = average; 
                }
                else
                {
                    //Y信号以外は固定値
                    imgout[k][i][j] = imgout[k][i - 1][j] = imgout[k][i][j + 1] = imgout[k][i - 1][j + 1] = 128; 
                }
            }
        }
    }
